...
if (empty($_POST['last_name']) || empty($_POST['dob'])) {
    echo "<p>You must input data into each field. 
        Click your browser's Back button to go to the delete form.</p>\n";
} else {
    $user = "root";
    $password = "root";
    $host = "localhost";
    ...
    if ($DBConnect === FALSE) {
        ...
        $dob = $_POST['dob']; 
        $SQLstring = "DELETE FROM $TableName WHERE last_name = '$last_name' AND date_of_birth = '$dob'"; 

        $QueryResult = mysqli_query($DBConnect, $SQLstring); 

        if (mysqli_num_rows($QueryResult) == 0) echo "<p>The appointment has been deleted.</p>";
        /* else {
            echo "<p>Appointment has been deleted*******.</p>"; 
...
?>
<p><a href="appointmentShow.php">View Appointments</a></p>
</body>
</html>